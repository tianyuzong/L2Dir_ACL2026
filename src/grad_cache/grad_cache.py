...
class GradCache:
    ...
    def __init__(
            ...
            get_rep_fn_unnorm: Callable[..., Tensor] = None,
            fp16: bool = False,
        ...
        self.get_rep_fn_unnorm = get_rep_fn_unnorm
    ...
    def interleave_blocks(self, list1, list2, block_size):
        result = []
        for i in range(0, len(list1), block_size):
            result.extend(list1[i:i + block_size])
            result.extend(list2[i:i + block_size])
        return result
    ...
    def get_reps_unnorm(self, model_out) -> Tensor:
        """
        Return representation tensor from generic model output
        :param model_out: generic model output
        :return: a single tensor corresponding to the model representation output
        """
        if self.get_rep_fn_unnorm is not None:
            return self.get_rep_fn_unnorm(model_out)
        else:
            return model_out
    ...
    def compute_loss(self, *reps: Tensor, **loss_kwargs) -> Tensor:
        ...
        """
        # print("lailailai", *reps, **loss_kwargs)
    ...
    def forward_no_grad(
        ...
        with torch.no_grad():
            for x in model_inputs:
                ...
                # print("y——————", y)
                # print("y_unnormed", y_unnormed)
                # print("x__", x)
                # print("y__", y)
                model_reps.append(self.get_reps(y))
                model_reps_unnorm.append(self.get_reps_unnorm(y))
        # concatenate all sub-batch representations
        model_reps = torch.cat(model_reps, dim=0)
        model_reps_unnorm = torch.cat(model_reps_unnorm, dim=0)
        # print("model_reps", model_reps.shape)
        return model_reps, model_reps_unnorm, rnd_states
    ...
    def forward_backward(
        ...
        for x, state, gradient, sync_context in zip(model_inputs, random_states, cached_gradients, sync_contexts):
            with sync_context():
                ...
                reps_unnorm = self.get_reps_unnorm(y)

                # print("is_equal",self.get_reps(y)==self.get_reps_unnorm(y))
                surrogate = torch.dot(reps.flatten(), gradient.flatten())
                surrogate_unnorm = torch.dot(reps_unnorm.flatten(), gradient.flatten())
                # print("gradient",gradient.shape)
                # print("surrogate",surrogate)
                # surrogate.backward()
                # allsurrogate=surrogate+surrogate_unnorm
                surrogate.backward()
    ...
    def cache_step(
        ...
        for model, x in zip(self.models, model_inputs):
            model_reps, model_reps_unnorm, rnd_states = self.forward_no_grad(model, x)
            all_reps.append(model_reps)
            # print("all_reps", )
            # print("ltn")
            all_reps_unnorm.append(model_reps_unnorm)
            # print("all_reps", len(all_reps), "all_reps_unnorm", len(all_reps_unnorm))
            # all_reps = self.interleave_blocks(all_reps, all_reps_unnorm, 2)
            all_rnd_states.append(rnd_states)
        # print("all_reps", all_reps)
        # print("all_reps__", all_reps)
        all_reps_ = self.interleave_blocks(all_reps, all_reps_unnorm, 2)
        cache, loss = self.build_cache(*all_reps_, **loss_kwargs)
