...
from transformers.modeling_outputs import BaseModelOutputWithPast, ModelOutput, BaseModelOutputWithPast_Unnorm
# from model_output import BaseModelOutputWithPast, ModelOutput
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    def forward(
            ...
            cache_position: Optional[torch.LongTensor] = None,
            *args, **kwargs
    ) -> Union[Tuple, BaseModelOutputWithPast_Unnorm]:
        ...
        hidden_states_unnorm = hidden_states.clone()
        ...
        return BaseModelOutputWithPast_Unnorm(
            last_hidden_states_unnorm=hidden_states_unnorm,
...
@dataclass
class Qwen2VLCausalLMOutputWithPast_unnorm(ModelOutput):
    """
    Base class for Qwen2VL causal language model (or autoregressive) outputs.
    ...
    past_key_values: Optional[List[torch.FloatTensor]] = None
    hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    last_hidden_states_unnorm: Optional[Tuple[torch.FloatTensor]] = None
    attentions: Optional[Tuple[torch.FloatTensor]] = None
    rope_deltas: Optional[torch.LongTensor] = None
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        # self.pooler_layer = torch.nn.Linear(
    ...
    @replace_return_docstrings(output_type=Qwen2VLCausalLMOutputWithPast_unnorm, config_class=_CONFIG_FOR_DOC)
    def forward(
            self,
            input_ids: torch.LongTensor = None,
            attention_mask: Optional[torch.Tensor] = None,
            ...
            cache_position: Optional[torch.LongTensor] = None,
            *args, **kwargs
    ) -> Union[Tuple, Qwen2VLCausalLMOutputWithPast_unnorm]:
        r"""
        Args:
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                image_grid_thw = [
                    image_grid_thw[i] if image_grid_thw[i] is not None and image_grid_thw[i].sum() > 0 else None for i
                    in range(bsz)]
                ...
                if len(idx_w_image) > 0:
                    ...
                    if len(idx_w_image):
                        ...
                        valid_pixel_values = [
                            pixel_values[i] if isinstance(pixel_values[i], torch.Tensor) else torch.from_numpy(
                                pixel_values[i]) for i in idx_w_image]
                        valid_pixel_values = torch.cat(valid_pixel_values).to(
                            input_ids.device)  # shape=[BS*n_patch,C*H*W]
                        ...
                        valid_grid_thw = [
                            image_grid_thw[i] if isinstance(image_grid_thw[i], torch.Tensor) else torch.from_numpy(
                                image_grid_thw[i]) for i in idx_w_image]
            ...
            if pixel_values_videos is not None:
                video_grid_thw = [
                    video_grid_thw[i] if video_grid_thw[i] is not None and video_grid_thw[i].sum() > 0 else None for i
                    in range(bsz)]
                ...
                if len(idx_w_video) > 0:
                    ...
                    if len(idx_w_video):
                        ...
                        valid_pixel_values = [pixel_values_videos[i] if isinstance(pixel_values_videos[i],
                                                                                   torch.Tensor) else torch.from_numpy(
                            pixel_values_videos[i]) for i in idx_w_video]
                        valid_pixel_values = torch.cat(valid_pixel_values).to(
                            input_ids.device)  # shape=[BS_w_video*n_patch,T*C*H*W], say T*C*H*W=1176=2*3*14*14
                        ...
                        valid_grid_thw = [
                            video_grid_thw[i] if isinstance(video_grid_thw[i], torch.Tensor) else torch.from_numpy(
                                video_grid_thw[i]) for i in idx_w_video]
                        valid_grid_thw = torch.cat(valid_grid_thw).to(input_ids.device)  # shape=[BS,H,W]
        ...
        # pooler_output = self.pooler_layer(hidden_states)
        # print("hidden_states", hidden_states.shape, pooler_output.shape)
        ...
        return Qwen2VLCausalLMOutputWithPast_unnorm(
            loss=loss,
            logits=logits,
            ...
            last_hidden_states_unnorm=outputs.last_hidden_states_unnorm,
        )
