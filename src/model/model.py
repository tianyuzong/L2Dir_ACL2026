...
class MMEBModel(nn.Module):
    ...
    def encode_input(self, input):
        if getattr(self, "model_backbone", None) == INTERNVIDEO2:
            ...
            texts = [text.replace(VLM_IMAGE_TOKENS[QWEN2_VL] + '\n', '') for text in
                     input["texts"]]  # we are actually passing video queries so this should not happen
            images = []
            for imgs in input['images']:
                # if multi images are given, select the middle frame only
                if isinstance(imgs, list):
                    imgs = imgs[len(imgs) // 2]
                    assert not isinstance(imgs,
                                          list)  # make sure we have extracted the middle frame and it is no longer a list
                    images.append(imgs)
                else:
                    images.append(imgs)
            ...
            # print("input", input)
            hidden_states = self.encoder(**input, return_dict=True, output_hidden_states=True)
            # print("hidden_states__", hidden_states.last_hidden_states_unnorm.shape)
            hidden_states_unnorm = hidden_states.last_hidden_states_unnorm
            # print("hidden_states_unnorm", hidden_states_unnorm.shape)
            hidden_states = hidden_states.hidden_states[-1]
            # print("hidden_states_unnorm", torch.norm(hidden_states_unnorm))
            # print("hidden_states", torch.norm(hidden_states))
            # print("hidden_states_unnorm", hidden_states_unnorm.shape)
            # assert hidden_states.shape == hidden_states_unnorm.shape
            pooled_output = self._pooling(hidden_states, input['attention_mask'])
            pooled_output_unnorm = self._pooling_unnorm(hidden_states_unnorm, input['attention_mask'])
            # print("pooled_output_unnorm", pooled_output_unnorm.shape)
            # pooled_output_unnorm
            # print("pooled_output", torch.norm(pooled_output - pooled_output_unnorm, dim=1))
            return pooled_output, pooled_output_unnorm
    ...
    def _pooling_unnorm(self, last_hidden_state, attention_mask):
        if self.pooling == 'last' or self.pooling == 'eos':
            left_padding = (attention_mask[:, -1].sum() == attention_mask.shape[0])
            batch_size = last_hidden_state.shape[0]
    ...
    def build(cls, model_args: ModelArguments, **kwargs):
        ...
        if model_backbone == PHI3V:
            ...
            print("backbone2model", backbone2model)
    ...
    def load(cls, model_args: ModelArguments, is_trainable=True, **kwargs):
        ...
        if model_args.model_backbone in {LLAVA_NEXT, QWEN2_VL, QWEN2_5_VL, QWEN2_VL_TOKENSELECTION,
                                         QWEN2_5_VL_TOKENSELECTION, E5_V}:
            config = AutoConfig.from_pretrained(model_args.model_name, trust_remote_code=True)
            config._attn_implementation = "flash_attention_2"
            ...
            config = AutoConfig.from_pretrained("src/model/vlm_backbone/internvideo2/",
                                                trust_remote_code=True)
            base_model = backbone2model[model_args.model_backbone].from_pretrained(
                "src/model/vlm_backbone/internvideo2/", config=config,
                trust_remote_code=True)
        ...
        if model_args.lora:
            ...
            lora_model = PeftModel.from_pretrained(base_model, model_name_or_path, config=lora_config,
    ...
    def forward(self, qry: Dict[str, Tensor] = None, tgt: Dict[str, Tensor] = None, *args, **kwargs):
        # print("qry————", qry)
        qry_reps, qry_reps_unnorm = self.encode_input(qry) if qry else (None, None)  # (bsz_per_device, dim)
        tgt_reps, tgt_reps_unnorm = self.encode_input(tgt) if tgt else (None, None)  # (bsz_per_device, dim)
        # if qry_reps is not None:
        #     print("qry_reps_", qry_reps.shape)
        if qry_reps is None or tgt_reps is None:
            return {"qry_reps": qry_reps, "tgt_reps": tgt_reps, "qry_reps_unnorm": qry_reps_unnorm,
                    "tgt_reps_unnorm": tgt_reps_unnorm}
        # else:
        # print("565656_")
        # print("")
        if self.is_ddp:
        ...
        # print("all_qry_reps",all_qry_reps.shape)
