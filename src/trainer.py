...
from src.data.collator.train_collator import split_vlm_inputs, get_dense_rep, get_dense_rep_unnorm, \
...
from src.loss import (
    ...
    InfoTNLoss,
    DistributedInfoTNLoss,
    InfoTNRegularizedLoss,
    DistributedInfoTNRegularizedLoss,
)
...
def set_seed(seed: int):
    ...
    torch.backends.cudnn.benchmark = False  # 禁用自动优化（否则会引入随机性）
    os.environ['PYTHONHASHSEED'] = str(seed)  # Python 散列种子

    # 可选：启用 PyTorch 的确定性操作（可能影响性能）
...
class MMEBTrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        print(f"Rank {self.args.local_rank}: Random seed set to {self.args.seed}")
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if not isinstance(train_dataset, torch.utils.data.IterableDataset):
            ...
            print("Using IterableDataset for training... shuffle True, drop_last=True, prefetch_factor=None")
            dataloader_params["sampler"] = None
            dataloader_params["shuffle"] = False
            dataloader_params["drop_last"] = True
            dataloader_params[
                "prefetch_factor"] = None  # # tune on both prefetch_factor and persistent_workers will cause hang at epoch2
...
class GradCacheLateProcessTrainer(MMEBTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        loss_fn_cls = DistributedInfoTNLoss if self.is_ddp else InfoTNLoss
        # 消融：用正则替代 InfoTN。 / Ablation: replace InfoTN with norm regularization.
        # loss_fn_cls = DistributedInfoTNRegularizedLoss if self.is_ddp else InfoTNRegularizedLoss
        # print("loss_fn_cls", loss_fn_cls)
        ...
        self.gc = GradCache(
            ...
            get_rep_fn_unnorm=get_dense_rep_unnorm,
            fp16=self.args.fp16,
