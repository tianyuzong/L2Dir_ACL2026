...
def init_mixed_dataset(dataset_config, model_args, data_args, training_args):
    ...
    for data_idx, (global_dataset_name, dataset_config) in enumerate(dataset_config.items()):
        print("data_idx, (global_dataset_name, dataset_config)", data_idx, (global_dataset_name, dataset_config))
        train_dataset = AutoPairDataset.instantiate(model_args=model_args, data_args=data_args,
                                                    training_args=training_args, **dataset_config)
        print_master(
            f"\t\tDataset#{data_idx} (dataset_parser={dataset_config.get('dataset_parser', 'n/a')}): {global_dataset_name}, num_rows={train_dataset.num_rows}, prob={probs[data_idx] * 100.0}")
    ...
    if len(train_datasets) > 1:
        train_dataset = interleave_datasets(train_datasets, probabilities=probs, batch_size=interleave_batch_size,
                                            ...
                                            stopping_strategy=training_args.interleave_stopping_strategy)
    else:
