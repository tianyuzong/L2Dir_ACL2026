...
def main():
    ...
    print("666")
    hf_config = AutoConfig.from_pretrained(model_args.model_name, trust_remote_code=False)
    ...
    for dataset_idx, (dataset_name, task_config) in enumerate(dataset_configs.items()):
        ...
        if do_query or do_cand:
            ...
            full_eval_qry_dataset, corpus = AutoEvalPairDataset.instantiate(model_args=model_args, data_args=data_args,
                                                                            **task_config)
        ...
        if do_query:
            ...
            eval_qry_loader = DataLoader(eval_qry_dataset, batch_size=training_args.per_device_eval_batch_size,
                                         collate_fn=eval_qry_collator, num_workers=training_args.dataloader_num_workers)
            query_embeds, gt_infos = encode_embeddings(model, eval_qry_loader, training_args, model_args,
                                                       padded_qry_dataset, encode_side="qry",
        ...
        if do_cand:
            ...
            eval_cand_loader = DataLoader(eval_cand_dataset, batch_size=training_args.per_device_eval_batch_size,
                                          collate_fn=eval_cand_collator,
                                          num_workers=training_args.dataloader_num_workers)

            cand_embeds, all_cand_ids = encode_embeddings(model, eval_cand_loader, training_args, model_args,
                                                          padded_cand_dataset, encode_side="cand",
        ...
        if local_rank == 0:
            ...
            if rank_against_all_candidates:
                ...
                for qid, (ranked_candid, gt_info) in tqdm(enumerate(zip(ranked_candids, gt_infos)),
                    ...
                    rel_docids = gt_info["label_name"] if isinstance(gt_info["label_name"], list) else [
                        gt_info["label_name"]]
            ...
            metrics_to_report = task_config["metrics"] if task_config.get("metrics", None) is not None else ["hit",
                                                                                                             "ndcg",
                                                                                                             "precision",
                                                                                                             "recall",
                                                                                                             "f1",
